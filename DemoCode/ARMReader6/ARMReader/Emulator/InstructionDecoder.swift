...
class InstructionDecoder {
    func decode(_ instruction: UInt32, cpu: ARM64CPU, bus: SystemBus) throws {
        ...
        if isFloatingPointInstruction(instruction) {
            try executeFloatingPointInstruction(instruction, cpu: cpu, bus: bus)
            return
        }

        // 然后检查是否是SIMD指令
        if isSIMDInstruction(instruction) {
            try executeSIMDInstruction(instruction, cpu: cpu, bus: bus)
            return
        }
        ...
        case 0x0D: // 自定义SIMD到标量指令
            executeCustomSIMDToScalar(instruction, cpu: cpu)

        case 0xF9: // LDR指令(F94系列)
            try executeLoadRegisterF9(instruction, cpu: cpu, bus: bus)
            return
    ...
    // 判断是否为SIMD指令
    private func isSIMDInstruction(_ instruction: UInt32) -> Bool {
        // 提取主操作码
        ...
        if majorOpcode == 0x4E {
            if debugMode {
                print("  检测到SIMD数据处理指令: 0x\(String(format:"%08X", instruction))")
            }
            return true
        }

        // 对于0x4C指令(LD1/ST1)，确认为SIMD指令
        if majorOpcode == 0x4C {
            // 通过检查特征位确认这是SIMD加载/存储
            let qaFlag = (instruction >> 30) & 0x1
            let isVector = qaFlag == 1 // Q位为1表示这是向量操作
            if isVector || (instruction & 0xFF000000) == 0x4C000000 {
                if debugMode {
                    print("  检测到SIMD加载/存储指令: 0x\(String(format:"%08X", instruction))")
                }
                return true
            }
        }
        ...
        if majorOpcode == 0x0D {
            if debugMode {
                print("  检测到SIMD-标量转移指令: 0x\(String(format:"%08X", instruction))")
            }
            return true
        }

        // 检查是否为已知的特定SIMD指令
        if instruction == 0x4E210402 || instruction == 0x4E610C03 ||
           instruction == 0x4E211C04 || instruction == 0x4EA11C05 ||
           instruction == 0x4EA19C06 || instruction == 0x4E080C07 {
            if debugMode {
                print("  检测到已知特定SIMD指令: 0x\(String(format:"%08X", instruction))")
            }
            return true
        }
    ...
    // 判断是否是浮点指令
    private func isFloatingPointInstruction(_ instruction: UInt32) -> Bool {
        // 检查主操作码
        let opcode = (instruction >> 24) & 0xFF

        // 浮点加载/存储指令 (LDR/STR) - 0xBD, 0xFD 等
        if (opcode & 0xBF) == 0xBD || (opcode & 0xBF) == 0xFD {
            return true
        }

        // 浮点数据处理指令 (0x1E 或 0x1F 开头)
        if opcode == 0x1E || opcode == 0x1F {
            return true
        }

        // 浮点比较指令
        if (opcode & 0xDF) == 0x5E || (opcode & 0xDF) == 0x7E {
            return true
        }

        // 浮点到整数的转换指令
        if (opcode & 0xDF) == 0x5F || (opcode & 0xDF) == 0x7F {
            return true
        }

        // 检查特定的浮点指令
        if (instruction & 0xFF3FFFE0) == 0x1E202020 || // FADD
           (instruction & 0xFF3FFFE0) == 0x1E203820 || // FSUB
           (instruction & 0xFF3FFFE0) == 0x1E200820 || // FMUL
           (instruction & 0xFF3FFFE0) == 0x1E201820 {  // FDIV
            return true
        }
    ...
    // 执行浮点指令
    private func executeFloatingPointInstruction(_ instruction: UInt32, cpu: ARM64CPU, bus: SystemBus) throws {
        let debugMode = EmulatorDebugTools.shared.isDebugModeEnabled
        if debugMode {
            print("  执行浮点指令: 0x\(String(format:"%08X", instruction))")
        }

        let executor = FloatingPointExecutor(cpu: cpu, memory: bus.getMemory())

        do {
            try executor.execute(instruction)
        } catch {
            if debugMode {
                print("  浮点指令执行失败: \(error.localizedDescription)")
                // 打印更多调试信息
            }
            throw error
        }
    ...
    // 执行SIMD指令
    private func executeSIMDInstruction(_ instruction: UInt32, cpu: ARM64CPU, bus: SystemBus) throws {
        let debugMode = EmulatorDebugTools.shared.isDebugModeEnabled
        if debugMode {
            print("  执行SIMD指令: 0x\(String(format:"%08X", instruction))")
        }

        let executor = SIMDExecutor(cpu: cpu, memory: bus.getMemory())

        do {
            try executor.execute(instruction)
        } catch {
            if debugMode {
                print("  SIMD指令执行失败: \(error.localizedDescription)")

                // 打印更多调试信息
                let majorOpcode = (instruction >> 24) & 0xFF
                print("  - 主操作码: 0x\(String(format:"%02X", majorOpcode))")
                print("  - 完整指令: 0x\(String(format:"%08X", instruction))")

                if let simdError = error as? EmulatorError {
                    switch simdError {
                    case .unsupportedInstructionFormat(let format, let opcode, let details):
                        print("  - 格式错误: \(format), 操作码: 0x\(String(format:"%02X", opcode))")
                        print("  - 详情: \(details)")
                    default:
                        break
                    }
    ...
    // 执行自定义的SIMD到标量转移指令
    private func executeCustomSIMDToScalar(_ instruction: UInt32, cpu: ARM64CPU) {
        let rd = Int(instruction & 0x1F)
        ...
        if debugMode {
            print("执行 SIMD到标量: X\(rd) = V\(vn).B[\(index)] = \(byteValue)")
        }
    }

    // 添加对LDR指令(F94系列)的支持，以防调试指令无法移除
    private func executeLoadRegisterF9(_ instruction: UInt32, cpu: ARM64CPU, bus: SystemBus) throws {
        let rt = Int(instruction & 0x1F) // 目标寄存器
        ...
        let offset = UInt64(imm12) << 3 // 扩展为字节偏移
        let address = baseAddress + offset

        if debugMode {
            print("执行 LDR X\(rt), [X\(rn), #\(offset)]")
        }

        // 尝试读取内存
        do {
            let value = try bus.read(at: address, size: 8)
            cpu.setRegister(UInt32(rt), value: value)

            if debugMode {
                print("从地址 0x\(String(format:"%llX", address)) 加载 0x\(String(format:"%llX", value))")
            }
        } catch {
            if debugMode {
                print("警告: 内存地址 0x\(String(format:"%llX", address)) 无法读取，返回0")
            }
            // 如果是调试指令，简单返回0并继续
            cpu.setRegister(UInt32(rt), value: 0)
        }
    }
}
