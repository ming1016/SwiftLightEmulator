...
class ARM64Assembler {
    ...
    /// 生成SUB立即数指令
    static func subImmediate(rd: Int, rn: Int, immediate: UInt16) -> UInt32 {
        precondition(rd >= 0 && rd <= 31, "寄存器索引必须在0-31之间")
        precondition(rn >= 0 && rn <= 31, "寄存器索引必须在0-31之间")
        precondition(immediate <= 4095, "立即数必须≤4095")
        ...
        // 构造SUB immediate指令格式
        let imm12 = UInt32(immediate & 0xFFF)
        let instruction = 0xD1000000 | (imm12 << 10) | (UInt32(rn) << 5) | UInt32(rd)
        ...
        let extractedRn = (instruction >> 5) & 0x1F
        let extractedImm = (instruction >> 10) & 0xFFF
        print("SUB immediate指令验证:")
        print("  生成指令: 0x\(String(format: "%08X", instruction))")
        print("  解码: SUB X\(extractedRd), X\(extractedRn), #\(extractedImm)")
        #endif
    ...
    /// 生成ORR寄存器指令
    static func orrRegister(rd: Int, rn: Int, rm: Int) -> UInt32 {
        precondition(rd >= 0 && rd <= 31, "寄存器索引必须在0-31之间")
        precondition(rn >= 0 && rn <= 31, "寄存器索引必须在0-31之间")
        precondition(rm >= 0 && rm <= 31, "寄存器索引必须在0-31之间")

        return 0xAA000000 | UInt32(rd) | (UInt32(rn) << 5) | (UInt32(rm) << 16)
    }

    /// 生成EOR寄存器指令 (异或)
    static func eorRegister(rd: Int, rn: Int, rm: Int) -> UInt32 {
        precondition(rd >= 0 && rd <= 31, "寄存器索引必须在0-31之间")
        precondition(rn >= 0 && rn <= 31, "寄存器索引必须在0-31之间")
        precondition(rm >= 0 && rm <= 31, "寄存器索引必须在0-31之间")

        return 0xCA000000 | UInt32(rd) | (UInt32(rn) << 5) | (UInt32(rm) << 16)
    }

    /// 生成左移指令 (寄存器)
    static func lslRegister(rd: Int, rn: Int, rm: Int) -> UInt32 {
        precondition(rd >= 0 && rd <= 31, "寄存器索引必须在0-31之间")
        precondition(rn >= 0 && rn <= 31, "寄存器索引必须在0-31之间")
        precondition(rm >= 0 && rm <= 31, "寄存器索引必须在0-31之间")

        return 0xAB000000 | UInt32(rd) | (UInt32(rn) << 5) | (UInt32(rm) << 16)
    }

    /// 生成右移指令 (寄存器)
    static func lsrRegister(rd: Int, rn: Int, rm: Int) -> UInt32 {
        precondition(rd >= 0 && rd <= 31, "寄存器索引必须在0-31之间")
        precondition(rn >= 0 && rn <= 31, "寄存器索引必须在0-31之间")
        precondition(rm >= 0 && rm <= 31, "寄存器索引必须在0-31之间")

        return 0xAB010000 | UInt32(rd) | (UInt32(rn) << 5) | (UInt32(rm) << 16)
    }

    /// 生成左移指令 (立即数)
    static func lslImmediate(rd: Int, rn: Int, shift: UInt8) -> UInt32 {
        precondition(rd >= 0 && rd <= 31, "寄存器索引必须在0-31之间")
        precondition(rn >= 0 && rn <= 31, "寄存器索引必须在0-31之间")
        precondition(shift <= 63, "移位值必须≤63")

        return 0xD4000000 | UInt32(rd) | (UInt32(rn) << 5) | (UInt32(shift) << 10)
    }

    /// 生成右移指令 (立即数)
    static func lsrImmediate(rd: Int, rn: Int, shift: UInt8) -> UInt32 {
        precondition(rd >= 0 && rd <= 31, "寄存器索引必须在0-31之间")
        precondition(rn >= 0 && rn <= 31, "寄存器索引必须在0-31之间")
        precondition(shift <= 63, "移位值必须≤63")

        return 0xD4400000 | UInt32(rd) | (UInt32(rn) << 5) | (UInt32(shift) << 10)
    }

    /// 生成无符号除法指令
    static func udiv(rd: Int, rn: Int, rm: Int) -> UInt32 {
        precondition(rd >= 0 && rd <= 31, "寄存器索引必须在0-31之间")
        precondition(rn >= 0 && rn <= 31, "寄存器索引必须在0-31之间")
        precondition(rm >= 0 && rm <= 31, "寄存器索引必须在0-31之间")

        return 0x9AC00800 | UInt32(rd) | (UInt32(rn) << 5) | (UInt32(rm) << 16)
    }

    /// 生成有符号除法指令
    static func sdiv(rd: Int, rn: Int, rm: Int) -> UInt32 {
        precondition(rd >= 0 && rd <= 31, "寄存器索引必须在0-31之间")
        precondition(rn >= 0 && rn <= 31, "寄存器索引必须在0-31之间")
        precondition(rm >= 0 && rm <= 31, "寄存器索引必须在0-31之间")

        return 0x9AC00C00 | UInt32(rd) | (UInt32(rn) << 5) | (UInt32(rm) << 16)
    }
